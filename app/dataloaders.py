...
async def load_app_or_user(keys: list[str]) -> list[str]:
    app_keys = [k for k in keys if k.startswith("app_")]
    user_keys = [k for k in keys if k.startswith("u_")]
    apps = [
        item
        async for item in DeployedApp.objects.filter(id__in=app_keys).select_related(
            "owner"
        )
    ]
    users = [item async for item in User.objects.filter(id__in=user_keys)]
    item_map = {item.id: item for item in apps} | {item.id: item for item in users}
...
class Loader:
    def __init__(self):
        ...
        self.app_or_user = DataLoader(load_fn=load_app_or_user)
